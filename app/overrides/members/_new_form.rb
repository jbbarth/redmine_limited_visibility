...
Deface::Override.new :virtual_path => 'members/_new_form',
                     ...
                     :text => <<EOF
<% if Redmine::Plugin.installed?(:redmine_organizations) && User.current.managed_only_his_organization?(@project) %>
  <%= render_principals_for_new_members(@project, 100, User.current.organization) %>
<% else %>
  <%= render_principals_for_new_members(@project, 100) %>
<% end %>
EOF

...
require 'redmine_limited_visibility/member_patch'
...
describe PrincipalMembershipsController, type: :controller do
  ...
  it "should create user memberships with functions" do
    assert_difference 'Member.count', 2 do
      xhr :post, :create, :user_id => 7, :membership => {:project_ids => [3, 4], :role_ids => [2, 3], :function_ids => [contractor_role.id, project_office_role.id]}, :format => 'js'
  ...
  it "should create user memberships with functions (with strings as params)" do
    assert_difference 'Member.count', 2 do
      xhr :post, :create, :user_id => 7, :membership => {:project_ids => [3, 4], :role_ids => ["2"], :function_ids => [contractor_role.id.to_s]}, :format => 'js'
      expect(response).to be_success
      assert_template 'create'
      assert_equal 'text/javascript', response.content_type
    end
